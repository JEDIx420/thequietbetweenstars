...
export class FloatingOrigin {
  ...
  public checkAndRebase(
    ...
    // Rebase vector: shift everything by -shipRenderPos
    const shift = shipRenderPos.clone();

    // 1. Update logical world position before shifting render coordinates
    worldPos.add(shift);

    // 2. Shift ship back to local origin (0, 0, 0)
    shipRenderPos.set(0, 0, 0);

    // 3. Shift all registered 3D root objects by -shift
    ...
    // 4. Notify listeners (such as camera target positions, particles, etc.)
